    ...
    internal class Program
    {
        ...
        private static void Main(string[] args)
        {
            ...
            //{
                ...
                var imagesPath = @"..\..\ImageSamples\";
                var fileName = imagesPath + "sample.bmp";
                //var fileName = imagesPath + "32.bmp";
                //var fileName = imagesPath + "Big_Black_River_Railroad_Bridge.bmp";
                ...
                using (var fileStream = File.OpenRead(fileName))
                    using (var bmp = (Bitmap) Image.FromStream(fileStream, false, false))
                    {
                        ...
                        var imageMatrix = new YCbCrImage(bmp);

                        sw.Stop();
                        Console.WriteLine($"{bmp.Width}x{bmp.Height} - {fileStream.Length / (1024.0 * 1024):F2} MB");
                        sw.Start();

                        var compressionResult = FastCompress(imageMatrix, CompressionQuality);
                ...
                var uncompressedImage = (Bitmap) FastUncompress(compressedImage);
        ...
        private static CompressedImage FastCompress(YCbCrImage image, int quality = 50)
        {
            var allQuantizedBytes = new List<byte>();
            var selectors = new Func<YCbCrImage, double[,]>[] {i => i.Y, i => i.Cb, i => i.Cr};

            for (var y = 0; y < image.Height - DCTSize + 1; y += DCTSize)
                for (var x = 0; x < image.Width - DCTSize + 1; x += DCTSize)
                    foreach (var selector in selectors)
                    {
                        var subMatrix = FastGetSubMatrix(selector(image), y, DCTSize, x, DCTSize);
                        ShiftMatrixValues(subMatrix, -128);
                        var channelFreqs = SFFT.FFT2D(subMatrix);
                        var quantizedFreqs = FastQuantize(channelFreqs, quality);
        ...
        private static YCbCrImage FastUncompress(CompressedImage image)
        {
            var result = new YCbCrImage(image.Height, image.Width);
                ...
                new MemoryStream(HuffmanCodec.Decode(image.CompressedBytes, image.DecodeTable, image.BitsCount)))
            {
                ...
                {
                    var channels = new List<double[,]>(3);
                    for (var i = 0; i < 3; i++)
                    {
                        ...
                        var channel = SFFT.IFFT2D(channelFreqs);
                        ShiftMatrixValues(channel, 128);
                        channels.Add(channel);
                    }
                    FastSetPixels(result, channels, y, x);
                }
            }

            return result;
        }
        private static double[,] FastGetSubMatrix(double[,] component, int yOffset, int yLength, int xOffset, int xLength)
        {
            var result = new double[yLength, xLength];
            for (var j = 0; j < yLength; j++)
                for (var i = 0; i < xLength; i++)
                    result[j, i] = component[yOffset + j, xOffset + i];
            return result;
        }

        private static byte[,] FastQuantize(double[,] channelFreqs, int quality)
        {
            ...
            for (var x = 0; x < width; x++)
                result[y, x] = (byte)(channelFreqs[y, x] / quantizationMatrix[y, x] * DCTSize);

            return result;
        }

        private static void FastSetPixels(YCbCrImage iamge, List<double[,]> channels, int yOffset, int xOffset)
        {
            ...
            for (var i = 0; i < height; i++)
                ...
                {
                    iamge.Y[yOffset + i, xOffset + j] = channels[0][i, j];
                    iamge.Cb[yOffset + i, xOffset + j] = channels[1][i, j];
                    iamge.Cr[yOffset + i, xOffset + j] = channels[2][i, j];
                }
